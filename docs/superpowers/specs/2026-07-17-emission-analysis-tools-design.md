...
每个 Agent 工具对应一个明确的业务意图。工具内部可以执行准备查询，例如把拠点名称解析为 `baseId`，但每轮最多执行一个最终业务分析工具。这种设计让 LLM 的可选工具和参数保持清晰，同时让审计记录能直接表达用户执行的分析类型。

## 调用流程

1. LLM 从消息和页面上下文提取工具名、会社、年度、Scope、期间以及拠点名称或 ID。
2. Gateway 验证目标会社属于登录用户的自社或直接子会社。
3. 涉及拠点时，Gateway 调用 `GET /analysis/baseInfoByCompanyGroup` 获取该会社的合法拠点集合。
4. Gateway 将名称解析成 `baseId`，并验证明确提供的 `baseId` 也存在于合法集合中。
5. 参数完整且唯一匹配后，Gateway 使用原 Keycloak Token 调用对应 CMPF Carbon API。
6. Gateway 把 CMPF DTO 确定性转换为受限 `ChartSpec` 和日英双语摘要。
7. Gateway 持久化助手消息、图表和审计记录，并通过 SSE 返回结果。
